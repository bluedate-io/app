...
const GENDER_NOT_PROVIDED = "__not_provided__";
...
function ReminderHistorySheet({
  ...
  return (
    ...
    >
      <div
        className="flex h-full w-full max-w-md flex-col border-l-2 bg-white shadow-[-6px_0_24px_-8px_rgba(45,26,14,0.18)]"
      ...
      >
        ...
        >
          <div className="min-w-0">
            <p
              ...
              style={{ color: DARK, fontFamily: "var(--font-bd-display), Georgia, serif" }}
            >
              Opt-in reminder history
            </p>
          ...
          <button
            type="button"
            onClick={onClose}
            className="ml-3 shrink-0 rounded-lg p-2 transition hover:bg-bd-table-hover"
          >
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {!loading && !error && data && data.events.length === 0 && (
            <p className="text-sm" style={{ color: SUBTLE }}>
              No opt-in reminders have been sent to this user.
            </p>
          )}
          {!loading && !error && data && data.events.length > 0 && (
              ...
              {data.events.map((ev) => (
                ...
                >
                  ...
                  <p className="text-xs" style={{ color: MUTED }}>
                    Batch of {ev.batchBccCount} BCC
                    {ev.sentByName?.trim() ? ` · by ${ev.sentByName}` : ""}
                  </p>
                </li>
              ))}
            </ul>
...
function ConfirmSendModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm rounded-2xl border-2 bg-white p-6 shadow-[5px_5px_0px_0px_var(--bd-shadow-ink)]"
        style={{ borderColor: adminTheme.inkDark }}
      >
        <p
        ...
        <p className="mb-5 text-sm" style={{ color: MUTED }}>
          This will send a BCC email to <strong style={{ color: DARK }}>{selectedCount}</strong>{" "}
          user{selectedCount === 1 ? "" : "s"} who haven&apos;t opted in for this week.
        </p>
...
export default function OptInReminderClient() {
  ...
  const committedGenderLabel =
    committedGender.trim() === GENDER_NOT_PROVIDED
      ? "Not provided"
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/opt-in-reminder?${params.toString()}`, {
        credentials: "include",
      });
    ...
    void (async () => {
      try {
        const res = await fetch(
          `/api/admin/opt-in-reminder/users/${encodeURIComponent(userId)}/reminder-history`,
          { credentials: "include" },
        );
        const json = await res.json().catch(() => ({}));
        if (cancelled) return;
  ...
  const toggleSelectAllNotOptedIn = () => {
  ...
  const sendReminders = async () => {
    ...
    try {
      const body = selectAllMatching
        ...
        : { userIds: [...selected] };
      const res = await fetch("/api/admin/opt-in-reminder/send", {
        method: "POST",
        credentials: "include",
    ...
    <div className="mx-auto max-w-6xl px-6 py-8">
      ...
      <div className={`${ADMIN_ELEVATED_PANEL} mb-8 space-y-5`}>
        <div>
          ...
          <p className="text-sm" style={{ color: SUBTLE }}>
            Users who are onboarded but haven&apos;t opted in for this week. Select rows and send one BCC email.
          </p>
          {data != null && (
        ...
        >
          {lastSend ? (
            ...
            <p className="m-0" style={{ color: SUBTLE }}>
              No opt-in reminder emails have been sent yet.
            </p>
          )}
        </div>
      </div>
      ...
      >
        <div className="flex min-w-0 flex-1 flex-col gap-2 sm:flex-row sm:items-center">
          <label className="sr-only" htmlFor="opt-in-reminder-search">
            Search by name or email
          </label>
          <input
            id="opt-in-reminder-search"
            type="search"
            value={searchDraft}
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0">
          <div className="flex items-center gap-2">
            <label htmlFor="opt-in-reminder-gender" className="text-sm whitespace-nowrap" style={{ color: MUTED }}>
              Gender
            </label>
            <select
              id="opt-in-reminder-gender"
            ...
            >
              ...
              <option value={GENDER_NOT_PROVIDED}>Not provided</option>
          ...
          <div className="flex items-center gap-2">
            <label htmlFor="opt-in-reminder-sort" className="text-sm whitespace-nowrap" style={{ color: MUTED }}>
              Sort by
            </label>
            <select
              id="opt-in-reminder-sort"
              value={sort}
              onChange={(e) => {
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <button
          type="button"
          onClick={toggleSelectAllNotOptedIn}
          disabled={loading || (!selectAllMatching && emailableTotal === 0)}
          className={ADMIN_BTN_NEUTRAL}
        >
          {selectAllMatching ? "Deselect all not opted in" : "Select all not opted in"}
        </button>
      ...
      {selectAllMatching && emailableTotal > 0 && (
        <p className="text-sm mb-4" style={{ color: MUTED }}>
          All {emailableTotal} not opted-in user{emailableTotal === 1 ? "" : "s"} with an email
        ...
        {loading ? (
          ...
          <p className="p-6 text-sm" style={{ color: SUBTLE }}>
            {committedQ.trim()
              ...
              : "All users have opted in for this week."}
          </p>
        ) : (
