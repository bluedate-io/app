...
export type OptInReminderListSort = z.infer<typeof sortSchema>;

const GENDER_NOT_PROVIDED = "__not_provided__";

/** Empty string = all genders; otherwise Woman | Man | Nonbinary | Not provided. */
export function normalizeOptInReminderGender(raw: string | null | undefined): string {
  const g = (raw ?? "").trim();
  if (g === "") return "";
  if (g === GENDER_NOT_PROVIDED) return GENDER_NOT_PROVIDED;
  return ADMIN_GENDER_OPTIONS.includes(g as (typeof ADMIN_GENDER_OPTIONS)[number]) ? g : "";
}

export function parseOptInReminderListQuery(searchParams: URLSearchParams) {
  ...
  const gender = normalizeOptInReminderGender(searchParams.get("gender"));
...
/** Normalizes POST body: either `{ userIds }` or `{ selectAllMatching, q?, gender? }`. */
export function parseOptInReminderSendBody(body: unknown):
  ...
  if (raw && raw.selectAllMatching === true) {
    ...
    const gender = normalizeOptInReminderGender(parsed.gender ?? "");
    return {
