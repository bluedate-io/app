// ─── AdminOptInReminderRepository — persistence for opt-in reminders ──────────
...
export type OptInIncompleteUserListRow = {
  id: string;
  email: string | null;
  createdAt: Date;
...
export class AdminOptInReminderRepository {
  constructor(private readonly db: PrismaClient) {}

  private static readonly GENDER_NOT_PROVIDED = "__not_provided__";
  ...
  private normalizeGenderFilter(gender?: string): string | undefined {
    ...
    if (g === AdminOptInReminderRepository.GENDER_NOT_PROVIDED) return g;
    return this.isAllowedGender(g) ? g : undefined;
  }

  /** Users who are onboarded, not admin, and have NOT opted in for the current week. */
  private notOptedInWhere(search: string, gender?: string): Prisma.UserWhereInput {
    const currentWeekStart = getWeekStartIST();
    const parts: Prisma.UserWhereInput[] = [
      { role: { not: UserRole.admin } },
      { onboardingCompleted: true },
      { weeklyOptIns: { none: { weekStart: currentWeekStart } } },
    ];
    const g = this.normalizeGenderFilter(gender);
    if (g) {
      if (g === AdminOptInReminderRepository.GENDER_NOT_PROVIDED) {
        parts.push({
          OR: [
            { preferences: { is: null } },
  ...
  private emailableNotOptedInWhere(search: string, gender?: string): Prisma.UserWhereInput {
    return {
      AND: [
        this.notOptedInWhere(search, gender),
        { email: { not: null } },
        { NOT: { email: { equals: "" } } },
      ],
    };
  ...
  async getReminderStatsForUserIds(
    ...
    >`
      ...
      LEFT JOIN "admin_opt_in_reminder_sends" s ON s."recipientUserIds" @> ARRAY[u.uid]::text[]
      GROUP BY u.uid
    `;
  ...
  async findRecentSendsWithRecipients(limit: number) {
    const rows = await this.db.adminOptInReminderSend.findMany({
      orderBy: { sentAt: "desc" },
      take: limit,
      select: {
        id: true,
  ...
  async getEmailableNotOptedInUserIds(q: string, gender?: string): Promise<string[]> {
    const search = q.trim();
    const rows = await this.db.user.findMany({
      where: this.emailableNotOptedInWhere(search, gender),
      select: { id: true, email: true },
    });
    return rows.filter((r) => r.email?.trim()).map((r) => r.id);
  }

  async listNotOptedInUsers(
    page: number,
    pageSize: number,
    ...
    const where = this.notOptedInWhere(search, genderParam);
    const [total, emailableRows] = await Promise.all([
      this.db.user.count({ where }),
      this.db.user.findMany({
        where: this.emailableNotOptedInWhere(search, genderParam),
        select: { email: true },
      }),
    ]);
    const emailableTotal = emailableRows.filter((r) => r.email?.trim()).length;

    let orderedUsers: OptInIncompleteUserListRow[];
    ...
    if (sort === "joined_desc" || sort === "joined_asc") {
      ...
      const genderSql =
        genderParam != null
          ? genderParam === AdminOptInReminderRepository.GENDER_NOT_PROVIDED
            ? Prisma.sql`AND (
                NOT EXISTS (
      ...
      if (pattern) {
        idRows = await this.db.$queryRaw<IdRow[]>`
          ...
          LEFT JOIN LATERAL (
            ...
            FROM "admin_opt_in_reminder_sends" s
            WHERE s."recipientUserIds" @> ARRAY[u.id]::text[]
          ) AS counts ON true
          WHERE u.role <> 'admin'::"UserRole"
            AND u."onboardingCompleted" = true
            AND NOT EXISTS (
              SELECT 1 FROM weekly_opt_ins w
              WHERE w."userId" = u.id AND w."weekStart" = ${weekStart}
            )
            ${genderSql}
            AND (
  ...
  async findLastSend() {
    return this.db.adminOptInReminderSend.findFirst({
      orderBy: { sentAt: "desc" },
      include: {
        sentBy: { select: { profile: { select: { fullName: true } } } },
      },
  ...
  findSendsHavingRecipient(userId: string) {
    return this.db.adminOptInReminderSend.findMany({
  ...
  createReminderSend(data: {
    ...
    return this.db.adminOptInReminderSend.create({ data });
  }
}
